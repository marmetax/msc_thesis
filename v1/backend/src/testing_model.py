...
actual_classes = df.iloc[:, 0].tolist()  # Get the actual classes from the last column
predictions = {}
...
for i in range(len(df)):
    ...
    dd = dd.replace(" ", ",")
    print(i, dd)
    str_results += "diagnosis(" + dd + "), " + dd[0] + ": "
    query = "\nquery(diagnosis(" + "A" + dd[1:] + "))."
    ...
    for result in results:
        ...
        print("r: ", r)
        predictions[i, str(result)[10]] = r
        str_results += " " + str(result)[10] + ": " + str(r) + ","
...
precision = precision_score(y_true, y_pred, pos_label='y', average='micro')
recall = recall_score(y_true, y_pred, pos_label='y', average='micro')
f1 = f1_score(y_true, y_pred, pos_label='y', average='micro')
...
with open("../dataset/testing_results.txt", "w") as results_file:
